...
const GraveEditDialog = lazy(() => import('./admin/GraveEditDialog'));
...
export default function GraveResultsList({ results = [], publicResults = [], isAdmin = false, showCount = true }: GraveResultsListProps) {
  ...
  const handleEditGrave = (grave: GraveRecord) => {
  ...
  const handleCloseEditDialog = (open: boolean) => {
    if (!open) {
      ...
      setTimeout(() => setEditingGrave(null), 150);
    }
  };
